...
object RegularExpressUtil {
  def main(args: Array[String]): Unit = {
    println(isValid("赣A12345"))
    val pattern1: Pattern = Pattern.compile("^[1-9]{2}[A-Z]{3}$")
    println(pattern1.matcher("12ASD").matches())
    val pattern2: Pattern = Pattern.compile("^[1-9]{2}[A-Za-z]{3}$")
    println(pattern2.matcher("12Asd").matches())
    val pattern3 = Pattern.compile( "^[\u4e00-\u9fc5]{1}[A-Z]{1}[A-Z0-9]{5,6}$")
    println(pattern3.matcher("赣A123456").matches())
  }
