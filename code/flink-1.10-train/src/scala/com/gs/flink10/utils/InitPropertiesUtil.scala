...
object InitPropertiesUtil extends Serializable {
  ...
  /**
    * get rule's properties
    *
    * @return java.util.Properties
    */
  def initRulesProp: Properties = {
    val prop: Properties = new Properties
    val in: InputStream = getClass.getResourceAsStream("/rules.properties")
    if (in == null) {
      println("ERROR : rule's properties init failed in is null")
    }
    prop.load(new BufferedInputStream(in))
    println("success : rule's properties init ok")
    prop
  }
